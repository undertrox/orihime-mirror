...
*/
	...
	public int Jyougehyou_settei(Tenkaizu_Syokunin orite, Tensyuugou otta_Men_zu, Tensyuugou Smen_zu) {//js.Jyougehyou_settei(ts1,ts2.get(),ts3.get());
		...
		//“™‰¿?ðŒ?‚ð?Ý’è‚·‚é?B–_ib‚ð‹«ŠE‚Æ‚µ‚Ä—×?Ú‚·‚é2‚Â‚Ì–Êim1,im2‚ª—L‚é?ê?‡?A?Ü‚è?ô‚Ý?„’è‚µ‚½?ê?‡‚É
...
*/
		...
		System.out.println("?ã‰º•\?E?l“à?@s0‚É—D?æ?‡ˆÊ‚ð‚Â‚¯‚é");
		for (int i = 1; i <= Smensuu; i++) {
...
//20191012 				Smen_yuukou_suu=Smensuu;//’x‚¢‚ªŠmŽÀ//20191012
		...
		//—D?æ?‡ˆÊ‚ð‹t“]‚³‚¹‚é?B‚±‚ê‚ª—LŒø‚©‚Ç‚¤‚©‚Í•s–¾wwwww
...
//System.out.print("Smen—LŒø?”/Smensuu?@" );System.out.print(Smen_yuukou_suu );System.out.print("?^");System.out.println(Smensuu );
		if (Smen_yuukou_suu == 0) {
			return 1000;
		}//Smen_yuukou_suu==0‚Æ‚¢‚¤‚±‚Æ‚Í?A?ã‰º•\‚É?V‚½‚È?î•ñ‚ð‰Á‚¦‚éSmen‚ª‚È‚¢‚Æ‚¢‚¤‚±‚Æ?B‚·‚È‚í‚¿?A?ã‰º•\‚ª‚·‚×‚Ä–„‚Ü‚Á‚Ä‚¢‚é‚Æ‚¢‚¤‚±‚Æ‚ðŽ¦‚·‚½‚ß?A‚±‚êˆÈ?ã‚Ì?d‚È‚è?„’è‚Í•s—v?B
	...
	public int kanou_kasanari_sagasi_self(int ss) {
		...
		if (ss == Smen_yuukou_suu) {
			...
			if (s[ss].kanou_kasanari_sagasi(jg) == 1000) {//==0‚Æ‚¢‚¤‚±‚Æ‚Í?A‰Â”\‚È?d‚È‚è‚©‚½‚Æ‚È‚é?‡—ñ‚Í‘¶?Ý‚µ‚È‚¢?B?@==1000?@‚±‚ÌSmen‚Í?A–µ?‚‚Í‚È‚¢?ó‘Ô‚É‚È‚Á‚Ä‚¢‚é?B
				return 100;//?Ü‚è?ô‚Ý‰Â”\‚È?‡—ñ‘g‚Ý?‡‚í‚¹‚ªŒ©‚Â‚©‚Á‚½?B
			} else {
				return 0;
			}
		...
		if (s[ss].kanou_kasanari_sagasi(jg) == 1000) {//==0‚Æ‚¢‚¤‚±‚Æ‚Í?A‰Â”\‚È?d‚È‚è‚©‚½‚Æ‚È‚é?‡—ñ‚Í‘¶?Ý‚µ‚È‚¢?B?@==1000?@‚±‚ÌSmen‚Í?A–µ?‚‚Í‚È‚¢?ó‘Ô‚É‚È‚Á‚Ä‚¢‚é?B
			...
			while (kanou_kasanari_sagasi_self(ss + 1) == 0) {//ŽŸ‚ÌŒ…‚Å‰Â”\‚È?d‚È‚è‚©‚½‚Æ‚È‚é?‡—ñ‚Í‘¶?Ý‚µ‚È‚¢
				if (s[ss].susumu() == 0) {
					return 0;
				}//‚±‚ÌŒ…‚Å‚Í?i‚ß‚È‚¢?i?V‚½‚È?‡—ñ‚Í–³‚¢?j

			}

			return 1000;//?Ü‚è?ô‚Ý‰Â”\‚È?‡—ñ‘g‚Ý?‡‚í‚¹‚ªŒ©‚Â‚©‚Á‚½?B
...
//------------------------------------------------------------------------------------------------------
	...
	private int mujyun_Smen_motome() { //jg‚Í•Ï‰»‚·‚é?B
		...
		for (int ss = 1; ss <= Smen_yuukou_suu; ss++) {      //<<<<<<<<<<<<<<?‚‘¬‰»‚Ì‚½‚ß•Ï?X?B070417
			...
			if (kks == 0) {//kks==0‚Æ‚¢‚¤‚±‚Æ‚Í?A‰Â”\‚È?d‚È‚è‚©‚½‚Æ‚È‚é?‡—ñ‚Í‘¶?Ý‚µ‚È‚¢
...
// ---------------------------------------------------------------
	...
	Color L_color = Color.black;//?ü‚Ì?F
...
*/
	...
	public Memo getMemo_for_svg_with_camera(Tenkaizu_Syokunin orite, Tensyuugou otta_Men_zu, Tensyuugou Smen_zu) {//?Ü‚è?ã‚ª‚è?}(hyouji_flg==5)
		...
		if (camera.get_camera_kagami() == 1.0) {
			omote_ura = 0;
		}//ƒJƒ?ƒ‰‚Ì‹¾?Ý’è‚ª1‚È‚ç?A?Ü‚è?ã‚ª‚è?}‚Í•\•\Ž¦
		if (camera.get_camera_kagami() == -1.0) {
			omote_ura = 1;
		}//ƒJƒ?ƒ‰‚Ì‹¾?Ý’è‚ª-1(xŽ²‚Ì•„?†‚ð”½“])‚È‚ç?A?Ü‚è?ã‚ª‚è?}‚Í— •\Ž¦
		...
		//‚±‚±‚Ü‚Å‚Å?A?ã‰º•\‚Ì?î•ñ‚ªSmen‚ÌŠe–Ê‚É“ü‚Á‚½
		...
		for (int im = 1; im <= Smensuu; im++) {//im‚ÍŠeSmen‚Ì”Ô?†
			if (s0[im].getMenidsuu() > 0) {//Menidsuu‚ÍSmen(?Ü‚è?ô‚Ý?„’è‚µ‚Ä‚¦‚ç‚ê‚½?j‹à?}‚ð?×•ªŠ„‚µ‚½–Ê)‚Å?d‚È‚Á‚Ä‚¢‚éMen(?Ü‚è‚½‚½‚Þ‘O‚Ì“WŠJ?}‚Ì–Ê)‚Ì?”?B‚±‚ê‚ª0‚È‚ç?Aƒh?[ƒiƒc?ó‚ÌŒŠ‚Ì–Ê‚È‚Ì‚Å•`‰æ‘Î?ÛŠO
				...
				//?Ü‚è?ã‚ª‚è?}‚ð•`‚­‚Æ‚«‚Ìim”Ô–Ú‚ÌSmen‚Ì‘½ŠpŒ`‚Ì’¸“_‚Ì?À•W?iPC•\Ž¦?ã?j‚ð‹?‚ß‚é
				for (int i = 1; i <= Smen_zu.getTenidsuu(im) - 1; i++) {
				...
				x[0] = gx(t1.getx());
				y[0] = gy(t1.gety());
				//?Ü‚è?ã‚ª‚è?}‚ð•`‚­‚Æ‚«‚Ìim”Ô–Ú‚ÌSmen‚Ì‘½ŠpŒ`‚Ì’¸“_‚Ì?À•W?iPC•\Ž¦?ã?j‚ð‹?‚ß‚é‚Ì‚Í‚±‚±‚Ü‚Å
...
/*
		...
		if(i_anti_alias==0){
	  		g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_OFF);//ƒAƒ“ƒ`ƒFƒCƒŠƒAƒX?@ƒIƒt
			BasicStroke BStroke = new BasicStroke(1.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER);g2.setStroke(BStroke);//?ü‚Ì‘¾‚³‚â?ü‚Ì––’[‚ÌŒ`?ó
		}
*/
		...
		for (int ib = 1; ib <= Smen_zu.getBousuu(); ib++) {
			...
			if (oekaki_flg == 1) {//–_‚ð•`‚­?B
				...
				//g.drawLine( gx(s_tv.getax()),gy(s_tv.getay()),gx(s_tv.getbx()),gy(s_tv.getby())); //svg ‚É’¼?ü‚ð?o—Í
...
//System.out.println("?ã‰º•\?E?l?@oekaki_with_camera+++++++++++++++hyouji_yousiki = "+hyouji_yousiki);
	...
	public Memo getMemo_hariganezu_for_svg_kakidasi(Tenkaizu_Syokunin orite, Tensyuugou otta_Men_zu, Tensyuugou Smen_zu, int i_fill) {
		...
		for (int i_nbox = 1; i_nbox <= otta_Men_zu.getMensuu(); i_nbox++) {
			int im = 0;
			if (camera.get_camera_kagami() == -1.0) {//ƒJƒ?ƒ‰‚Ì‹¾?Ý’è‚ª-1(xŽ²‚Ì•„?†‚ð”½“])‚È‚ç?A?Ü‚è?ã‚ª‚è?}‚Í— •\Ž¦
...
/*
		int col_hiku=0;int colmax=255;int colmin=30;//colmax=255(?^‚Á”’)ˆÈ‰º?Acolmin=0(?^‚Á?•)ˆÈ?ã
		//Menidsuu_max‚Í1ˆÈ?ã‚Å‚ ‚é‚±‚Æ
...
//-------------------------------------------------------
	...
	public int bou_no_bangou_kara_kagenoaru_Smen_no_bangou_wo_motomeru(int ib, Tensyuugou Smen_zu, int omote_ura) {//–_‚Ì”Ô?†‚©‚ç?A‚»‚Ì–_‚Ì‰e‚ª”­?¶‚·‚éSmen ‚Ì”Ô?†‚ð‹?‚ß‚é?B‰e‚ª”­?¶‚µ‚È‚¢?ê?‡‚Í0‚ð•Ô‚·?B
			...
			for (int i = 1; i <= 10; i++) {
				for (int ia = 1; ia <= jg.getMensuu(); ia++) {
					for (int ib = 1; ib <= jg.getMensuu(); ib++) {
						...
						if (ia != ib) {
							if (jg.get(ia, ib) == 1) {//wiki‚ÌƒCƒ?ƒŒ?[ƒeƒBƒ“ƒO‚Ì?«ŠûƒNƒ‰ƒu24ŽQ?Æ
...
//System.out.println("*********  rating2()   *************");
		...
		int[] i_m_r = new int[jgms + 1];//Še–Ê‚Ì?®?”ƒŒ?[ƒeƒBƒ“ƒO?irating2()‚ÅŽg‚¤?j?j
		i_men_rating = i_m_r;
...
//System.out.println("*********   get_maketa_kazu_goukei_without_rated_men   *************");
		...
		for (int ism = 1; ism <= Smensuu; ism++) {
			...
			if (i_make >= top_men_id_ga_maketa_kazu_goukei_without_rated_men) {
				return i_make;
			}//20180306?‚‘¬‰»‚Ì‚½‚ß‚±‚Ì‚P?s‚ð“ü‚ê‚Ä‚¢‚é‚ª?A–{“–‚ÉŒø‰Ê‚ª‚ ‚é‚©‚Í•s–¾?B‚±‚Ì?s‚¾‚¯ƒRƒ?ƒ“ƒgƒAƒEƒg‚µ‚Ä‚à?³?í‚É‚Í“®‚­‚Í‚¸?B
