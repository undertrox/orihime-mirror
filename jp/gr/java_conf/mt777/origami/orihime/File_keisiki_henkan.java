...
public class File_keisiki_henkan {
    ...
    //public File_keisiki_henkan(){   } //ƒRƒ“ƒXƒgƒ‰ƒNƒ^
    ...
    Memo obj2orihime(Memo mem) {
        ...
        for (int ig = 1; ig <= mem.getGyousuu(); ig++) {
            //  System.out.print("v,f,#e‚Ì“Ç‚Ý‚±‚Ý..."); System.out.println(ig);
        ...
        for (int i = 1; i <= Boumax; i++) {
            ...
            //MemR.addGyou("?À•W,"+str.valueOf(t[b[i].getmae()].getx())+","
    ...
    Memo cp2orihime(Memo mem) {
        ...
        //  int isen=0;
        for (int ig = 1; ig <= mem.getGyousuu(); ig++) {
            //     System.out.print("cpƒtƒ@ƒCƒ‹‚Ì?s?‡”Ô..."); System.out.println(ig);
            if (mem.getGyou(ig).length() != 0) {
                StringTokenizer tk = new StringTokenizer(mem.getGyou(ig), " ");
                str = tk.nextToken();
...
*/
    ...
    Memo orihime2svg(Memo mem_tenkaizu, Memo mem_oriagarizu) {
        System.out.println("svg‰æ‘œ?o—Í");
        Memo MemR = new Memo();
/*
                int ibangou=0;
		Integer Ii    = new Integer(0);
