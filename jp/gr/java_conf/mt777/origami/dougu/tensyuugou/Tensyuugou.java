...
//Ten_p tp=new Ten_p();
	...
	ArrayList[] T_renketu;//t_renketu[i][j]はt[i]に連決しているTenの番???Bt[0]には?ATemの?狽i納?B

	int[][] Men_tonari;//Men_tonari[i][j]はm[i]とm[j]の境界のBouの番???Bm[i]とm[j]が隣り?№墲ﾈいときは0を格納?B
    ...
    */
	...
	//点がどの面の内部にあるかどうかを判定する?B0ならどの面の内部にもない?Aマイナスなら境界?????A?ｳの?狽ﾈら内部?B該当する面番??が複?狽?る???№ﾍ番??の?ｬさいほうが返される?B
	...
	//存?ﾝするとき1?Aしないなら0を返す?B面が凹多角形の???№ﾍ結果がおかしくなるので使わないこと
	...
	//2つのBouが平?sで一部または全部で?dなるときは1?Aそうでなければ0をかえす?B1点で?dなる???№ﾍ0を返す?B
	...
	//BouやMenの?ﾀ標の?ﾅ大値?A?ﾅ?ｬ値を??める?Bkantan_totu_naibu関?狽ﾉのみ用いる?Bkantan_totu_naibu関?狽gうなら?ﾜり?ﾝ??定毎にやる必要あり?B
