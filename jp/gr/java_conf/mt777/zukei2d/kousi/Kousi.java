...
public class Kousi {
    ...
    double d_kousi_haba = 200.0;//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<格子??(double)

    double d_kousi_a_nagasa = 1.0;
    double d_kousi_b_nagasa = 1.0;
    double d_kousi_kakudo = -90.0;

    double d_kousi_ax = 1.0;//格子の横方向の単位ベクトルのX?ｬ分の比率
    double d_kousi_ay = 0.0;//格子の横方向の単位ベクトルのY?ｬ分の比率

    double d_kousi_bx = 0.0;//格子の?c方向の単位ベクトルのX?ｬ分の比率
    double d_kousi_by = 1.0;//格子の?c方向の単位ベクトルのY?ｬ分の比率

    double okx0 = -200.0;//obiject系での格子のx?ﾀ標の原点
    double oky0 = +200.0;//obiject系での格子のy?ﾀ標の原点

    double taikakusen_max = 1.0;//単位胞の対角??の長いほう
    double taikakusen_min = 1.0;//単位胞の対角??の短いほう


    int i_kitei_jyoutai = 1;//基底?i格子?jの?ｵ=0は全域で無効だが?A格子??だけは既存端点への引き寄せ半径の?ﾝ定に使うので有効?A?ｵ=1は用紙内のみ有効?A?ｵ=2は全領域で有効


    //用紙の分割なしならkousi_zahyou[0から1]なのでkousi_bunkatu_suuは１?Akousi_bunkatu_suu
    //用紙の2分割ならkousi_zahyou[0,1,2]なのでkousi_bunkatu_suuは2?A
    //用紙の4分割ならkousi_zahyou[0,1,2,3,4]なのでkousi_bunkatu_suuは4?A
    ...
    Color kousi_color = new Color(230, 230, 230);//格子??の?F
    Color kousi_memori_color = new Color(180, 200, 180);//格子目?ｷり??の?F

    int kousi_senhaba = 1;//格子の????
...
//------
    ...
    public Ten get_sisuu(Ten t0) {//obj系?ﾀ標のTenから?A格子の指?狽ｾる
        //?s列 [d_kousi_ax, d_kousi_bx]によって[1]は格子ベクトルaに変換され?A[1]は格子ベクトルbに変換される?B
        //     [d_kousi_ay, d_kousi_by]        [0]?@?@?@?@?@?@?@ ?@?@?@?@?@?@[0]
        //この逆?s列によってobj系?ﾀ標のTenは格子の指?狽ﾉ変換される?B
        //?@?@?@?@?@
        //?s列の記??の定義
    ...
    // ----------------------------
    public int get_a_sisuu_min(Ten p_a, Ten p_b, Ten p_c, Ten p_d) {//obj?ﾀ標系の4つの点を指定し?A各点のaベクトルの指?狽謔??ｬさい?ｮ?狽ﾌ指?狽ｾる?B
        Ten p_a_sisuu = new Ten();
        p_a_sisuu.set(get_sisuu(p_a));//p_aの格子系の指??
        Ten p_b_sisuu = new Ten();
        p_b_sisuu.set(get_sisuu(p_b));//p_bの格子系の指??
        Ten p_c_sisuu = new Ten();
        p_c_sisuu.set(get_sisuu(p_c));//p_cの格子系の指??
        Ten p_d_sisuu = new Ten();
        p_d_sisuu.set(get_sisuu(p_d));//p_dの格子系の指??
    ...
    // ----------------------------
    public int get_a_sisuu_max(Ten p_a, Ten p_b, Ten p_c, Ten p_d) {//obj?ﾀ標系の4つの点を指定し?A各点のaベクトルの指?狽謔闡蛯ｫい?ｮ?狽ﾌ指?狽ｾる?B
        Ten p_a_sisuu = new Ten();
        p_a_sisuu.set(get_sisuu(p_a));//p_aの格子系の指??
        Ten p_b_sisuu = new Ten();
        p_b_sisuu.set(get_sisuu(p_b));//p_bの格子系の指??
    ...
    // ----------------------------
    public int get_b_sisuu_min(Ten p_a, Ten p_b, Ten p_c, Ten p_d) {//obj?ﾀ標系の4つの点を指定し?A各点のbベクトルの指?狽謔??ｬさい?ｮ?狽ﾌ指?狽ｾる?B
        Ten p_a_sisuu = new Ten();
        p_a_sisuu.set(get_sisuu(p_a));//p_aの格子系の指??
        Ten p_b_sisuu = new Ten();
        p_b_sisuu.set(get_sisuu(p_b));//p_bの格子系の指??
    ...
    // ----------------------------
    public int get_b_sisuu_max(Ten p_a, Ten p_b, Ten p_c, Ten p_d) {//obj?ﾀ標系の4つの点を指定し?A各点のbベクトルの指?狽謔闡蛯ｫい?ｮ?狽ﾌ指?狽ｾる?B
        Ten p_a_sisuu = new Ten();
        p_a_sisuu.set(get_sisuu(p_a));//p_aの格子系の指??
        Ten p_b_sisuu = new Ten();
        p_b_sisuu.set(get_sisuu(p_b));//p_bの格子系の指??
    ...
    //描画-----------------------------------------------------------------
    public void oekaki(Graphics g, Camera camera, int p0x_max, int p0y_max, int i_irokae) {    //i_irokae=1なら一定?狽ｲとに格子??の?Fを変える
        ...
        //格子??の描画
        //g2.setStroke(new BasicStroke(1.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER));//??の太さや??の末端の形??
        g2.setStroke(new BasicStroke((float) kousi_senhaba, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER));//??の太さや??の末端の形??


        if (jyoutai() != 0) {
            Ten p0_a = new Ten();
            p0_a.set((double) 0, (double) 0);//画面の?ｶ?繝JドのTV系?ﾀ標
            Ten p0_b = new Ten();
            p0_b.set((double) 0, (double) p0y_max);//画面の?ｶ下カドのTV系?ﾀ標
            Ten p0_c = new Ten();
            p0_c.set((double) p0x_max, (double) p0y_max);//画面の右下カドのTV系?ﾀ標
            Ten p0_d = new Ten();
            p0_d.set((double) p0x_max, (double) 0);//画面の右?繝JドのTV系?ﾀ標

            Ten p_a = new Ten();
            p_a.set(camera.TV2object(p0_a));//画面の?ｶ?繝Jドのobj系?ﾀ標
            Ten p_b = new Ten();
            p_b.set(camera.TV2object(p0_b));//画面の?ｶ下カドのobj系?ﾀ標
            Ten p_c = new Ten();
            p_c.set(camera.TV2object(p0_c));//画面の右下カドのobj系?ﾀ標
            Ten p_d = new Ten();
            p_d.set(camera.TV2object(p0_d));//画面の右?繝Jドのobj系?ﾀ標
