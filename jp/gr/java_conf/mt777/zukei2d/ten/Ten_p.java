package jp.gr.java_conf.mt777.zukei2d.ten;

//import java.util.*;
/*
public class Ten {//“_‚Ì?À•W‚â•ûŒüƒxƒNƒgƒ‹‚È‚Ç‚ğ‚ ‚ç‚í‚·‚Æ‚«‚É—p‚¢‚é

	double x,y;
 

	public Ten(Ten p){set(p);}//ƒRƒ“ƒXƒgƒ‰ƒNƒ^ 
	public Ten(){x=0;y=0;}//ƒRƒ“ƒXƒgƒ‰ƒNƒ^
	public Ten(double i,double j){x=i;y=j;} //ƒRƒ“ƒXƒgƒ‰ƒNƒ^
	public Ten(double a,Ten p,double b,Ten q){x=a*p.getx()+b*q.getx();y=a*p.gety()+b*q.gety();} //ƒRƒ“ƒXƒgƒ‰ƒNƒ^


  
	public void set(Ten p){ x=p.getx();y=p.gety();}
	//public void set(Ten p){double dx,dy; dx=p.getx();dy=p.gety();x=dx;y=dy;}
	public void set(double i,double j){ x=i;y=j;}
	public void set(double a,Ten p,double b,Ten q){x=a*p.getx()+b*q.getx();y=a*p.gety()+b*q.gety();}

	public void setx(double xx){x=xx;}
	public void sety(double yy){y=yy;}

	//public double getx(){double dx=x;return dx;}
	//public double gety(){double dy=y;return dy;}
	public double getx(){return x;}  
	public double gety(){return y;}  
 
	public void reset(){ x=0.0;y=0.0;}
  
// int getix(){return (int)x;}
// int getiy(){return (int)y;}

	public void heikou_idou(double x1,double y1) { x=x+x1; y=y+y1; }

	//‘¼‚Ì“_‚Æ‚Ì‹——£?idouble?j‚ğ‹?‚ß‚éŠÖ?”----------------------------------------------------
	public double kyori(Ten p){
		//double x1=p.getx(),y1=p.gety();
		//return Math.sqrt((x1-x)*(x1-x)+(y1-y)*(y1-y));

		double x1=p.getx()-x,y1=p.gety()-y;
		return Math.sqrt(x1*x1+y1*y1);

	}



	//‘¼‚Ì“_‚Æ‚Ì‹——£‚Ì2?æ?idouble?j‚ğ‹?‚ß‚éŠÖ?”----------------------------------------------------
	public double kyori2jyou(Ten p){
		double x1=p.getx()-x,y1=p.gety()-y;
		return x1*x1+y1*y1;
	}



        //©Ten‚ğŠî?€‚Æ‚µ‚Ä‚İ‚½‚Æ‚«?A‘¼‚Ì“_‚ÌˆÊ’u‚ğTen‚Å•Ô‚·?B
	public Ten tano_Ten_iti(Ten taten){
		Ten rten =new Ten();
		rten.setx(taten.getx()-x);
		rten.sety(taten.gety()-y);
		return rten;
	}

	public void idou(Ten addten){
		x=x+addten.getx();
		y=y+addten.gety();
	}



}

*/


//==========================================================

public class Ten_p extends Ten {//Ten_p‚ÍTen‚É‘I‘ğ?ó‘Ô‚È‚Ç‚Ì‹@”\’Ç‰Á‚µ‚½‚à‚Ì‚Å?AƒIƒŠƒqƒ?ver.3.037‚©‚ç’Ç‰Á?B“WŠJ?}?E?l‚Ì?‚Â“_?W?‡‚Åg‚¤?B

    byte i_sentaku = 0;//‘I‘ğ?ó‘Ô

    public void set_ten_sentaku_1() {
        i_sentaku = 1;
    }

    public void set_ten_sentaku_0() {
        i_sentaku = 0;
    }

    public void change_ten_sentaku() {
        if (i_sentaku == 1) {
            i_sentaku = 0;
        } else if (i_sentaku == 0) {
            i_sentaku = 1;
        }
    }

    public byte get_ten_sentaku() {
        return i_sentaku;
    }


}
